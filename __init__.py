from anki.template.template import Template, modifier
...
def render(self, template=None, context=None, encoding=None):
    ...
    result, changed = self.render_tags(template, context)
...
def _renderQA(self, data, qfmt=None, afmt=None):
    ...
    for (type, format) in (("q", qfmt), ("a", afmt)):
        ...
        html, changed = anki.template.render(format, fields) #replace everything of the form {{ by its value TODO check
        d["changed"] = changed
        d[type] = runFilter(
...
def isEmpty(self):
    return not self._getQA()["changed"]
